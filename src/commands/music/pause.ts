...
export default class PauseCommand extends BaseCommand {
    ...
    async run(ctx: GuildCTX, opts?: Success | Error) {
        ...
        if (!res.dispatcher?.playing || !res.dispatcher.queue.current) return await ctx.reply({ embeds: [EmbedUtils.embedifyString(ctx.guild, "There is nothing playing right now!", { isError: true })] });
